function [num] = verify_gesture_p4(img_orig)
    ...
    % if the binary image at centroid is 0, return -3 (O exist)
    processed_img_filled = imfill(processed_img,'holes');
    area_filled = sum(sum(processed_img_filled));
    if (area_filled ~= area && bweuler(processed_img)==0)
        num = -3;
